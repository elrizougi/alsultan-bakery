                ...
                <div className="space-y-6">
                  ...
                  <div className="mt-6 p-4 bg-green-50 rounded-lg border-2 border-green-300">
                    <h4 className="text-lg font-bold text-green-700 mb-4 flex items-center gap-2">
                      ...
                      استلام مبلغ من المندوب
                    </h4>
                    {!isSettlementDepositOpen ? (
                      <Button 
                        onClick={() => {
                          setSettlementDepositAmount(parseFloat(settlementBalance?.cashBalance || '0').toFixed(2));
                          setIsSettlementDepositOpen(true);
                        }}
                        className="w-full bg-green-600 hover:bg-green-700 text-lg py-6"
                        data-testid="button-open-settlement-deposit"
                      >
                        <DollarSign className="h-5 w-5 ml-2" />
                        استلام مبلغ من الرصيد الحالي
                      </Button>
                    ) : (
                      <div className="space-y-4">
                        <div>
                          <Label className="mb-2 block">المبلغ المراد استلامه</Label>
                          <Input
                            type="number"
                            step="0.01"
                            min="0"
                            max={parseFloat(settlementBalance?.cashBalance || '0')}
                            value={settlementDepositAmount}
                            onChange={(e) => setSettlementDepositAmount(e.target.value)}
                            className="text-xl font-bold text-center"
                            data-testid="input-settlement-deposit-amount"
                          />
                        </div>
                        <div>
                          <Label className="mb-2 block">ملاحظات (اختياري)</Label>
                          <Input
                            value={settlementDepositNotes}
                            onChange={(e) => setSettlementDepositNotes(e.target.value)}
                            placeholder="ملاحظات..."
                            data-testid="input-settlement-deposit-notes"
                          />
                        </div>
                        <div className="flex gap-2">
                          <Button
                            onClick={handleSettlementDeposit}
                            disabled={!settlementDepositAmount || parseFloat(settlementDepositAmount) <= 0 || settlementDepositMutation.isPending}
                            className="flex-1 bg-green-600 hover:bg-green-700 py-6 text-lg"
                            data-testid="button-confirm-settlement-deposit"
                          >
                            <Check className="h-5 w-5 ml-2" />
                            {settlementDepositMutation.isPending ? "جاري الاستلام..." : "تأكيد الاستلام"}
                          </Button>
                          <Button
                            variant="outline"
                            onClick={() => {
